...
///         new GovernanceToken + StakedGovernanceToken (voting wrapper) +
///         Treasury + Governance set, wired together. Run this once per DAO
///         you want to launch.
///
/// Note: holders receive the initial supply as raw GovernanceToken, which
/// carries zero voting power on its own. Each holder must approve and call
/// `stake()` on the printed StakedGovernanceToken (UnderlyingToken in the
/// log output below refers to the raw token being staked) before they can
/// propose or vote.
///
/// Required env vars:
