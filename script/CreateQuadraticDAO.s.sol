...
/// @title CreateQuadraticDAO
/// @notice Calls `createDAO` on an already-deployed QuadraticDAOFactory.
///
/// Required env vars:
///   FACTORY_ADDRESS   - address of the already-deployed QuadraticDAOFactory
...
///   QUORUM_BPS              - default 1000  (10%, of sqrt(totalSupply))
...
///   PROPOSAL_THRESHOLD      - default 0 (whole tokens, raw balance - not sqrt-transformed)
///
/// Usage:
...
///
///   forge script script/CreateQuadraticDAO.s.sol:CreateQuadraticDAO \
///     --rpc-url <RPC_URL> \
///     --private-key $PRIVATE_KEY \
///     --broadcast
contract CreateQuadraticDAO is Script {
    function run() external returns (address governance) {
        ...
        QuadraticGovernance.QuadraticGovernanceConfig memory config = QuadraticGovernance.QuadraticGovernanceConfig({
        ...
        QuadraticDAOFactory factory = QuadraticDAOFactory(factoryAddress);
